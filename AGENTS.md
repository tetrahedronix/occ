...
1. **`occ` (planned)** — a Go CLI that renders `sandbox.yml` into a `Dockerfile` + shell launcher via embedded `text/template` templates. **No Go code exists yet.** The authoritative design is `TODO.org` (roadmap, tech stack, template conventions). Read it before writing code.
...
- `*.yml`, `*.yaml`, and `*.tmpl` are **not** unignored — the planned `sandbox.yml` and `Dockerfile.tmpl`/`sandbox-occ.tmpl` will be silently untracked unless you extend `.gitignore` first.
...
- Generated files get a provenance header (occ version + hash of `sandbox.yml`).
